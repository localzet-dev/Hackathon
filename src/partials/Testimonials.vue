      ...
      <div class="relative pb-12 md:pb-20">
        ...
        <div class="text-center">
            ...
            >              
              <template :key="index" v-for="(item, index) in items">
                <div v-show="active === index">
                  <div class="text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-neutral-200/60 via-neutral-200 to-neutral-200/60">{{ item.quote }}</div>
                </div>
              </template>
            </transition-group>
          </div>
          ...
          <div class="flex flex-wrap justify-center -m-1.5">
            <template :key="index" v-for="(item, index) in items">
              <button class="btn-sm m-1.5 text-xs py-1.5 text-neutral-300 transition duration-150 ease-in-out [background:linear-gradient(theme(colors.neutral.900),_theme(colors.neutral.900))_padding-box,_conic-gradient(theme(colors.slate.400),_theme(colors.slate.700)_25%,_theme(colors.slate.700)_75%,_theme(colors.slate.400)_100%)_border-box] relative before:absolute before:inset-0 before:bg-slate-800/30 before:rounded-full before:pointer-events-none" :class="active === index ? 'opacity-100' : 'opacity-30 hover:opacity-60'" @click="active = index; stopAutorotate();">
                <span class="relative">
